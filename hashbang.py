...
def optionalarg(decorator):
    '''
    Transforms a decorator `decorator(func, args)` into a decorator that can be executed both with
...
class CommandSpec:
    ...
    def add_arguments(self, parser):
        ...
        for argname, param in self.signature.parameters.items():
            if argname == 'self': continue  # Skip "self" arguments

            def add_argument(*args, **kwargs):
                '''
                Support inverted control: By adding parameter to the decorator and implementing the
                add function.
            ...
            name = argname.rstrip('_')  # Allow adding '_' at the end to avoid keyword collision
            if param.kind is Parameter.POSITIONAL_ONLY or param.kind is Parameter.POSITIONAL_OR_KEYWORD:
                ...
                if argname == '__rest__':
                    # Special argument that will capture any remaining entries in argv
                    # e.g. def run(*__rest__)
                ...
                if type(param.default) is bool:
                    # Flag to store true or false: def run(*, wipe=True, verbose=False)
                    # Generates --wipe and --nowipe
                    add_argument('--' + name, action='store_true', default=param.default, dest=argname)

                    # TODO: Need to allow Argument customization of this?
                    # (i.e. support --nof in addition to --nofile)?
                    parser.add_argument('--no' + name, action='store_false', dest=argname, default=param.default, help=argparse.SUPPRESS)
                else:
                    # Capture any other keyword arguments: def run(*, **kwargs)
                    # e.g. prog --arg1 val1 --arg2 val2
                    add_argument('--' + name, action='store', default=param.default, dest=argname)
            else:
                raise Exception('Unsupported kind of argument ' + str(param.kind))
    ...
    def get_args(self, opts, remaining):
        ...
        for argname, param in self.signature.parameters.items():
            ...
            elif param.kind is Parameter.POSITIONAL_ONLY or param.kind is Parameter.POSITIONAL_OR_KEYWORD:
    ...
    def parse_doc(func):
        ...
        if doc is None:
            return (None, None, {})
        description, usage = pad(re.split('usage: ', doc, flags=re.IGNORECASE), 2)
...
def _commanddelegator(func, args=(), *, prog=None):
    ...
    If you are implementing a delegator, the implementation should either call .execute() on another
    command, or raise NoMatchingDelegate exception. Any other side-effects are undesired.
    '''
...
def prefix_filter(choices, cword):
    return [choice for choice in choices if choice.lower().startswith(cword.lower())]

def substring_filter(choices, cword):
    return [choice for choice in choices if cword.lower() in choice.lower()]

def fuzzy_path_filter(choices, cword):
    def path_match(subpath, fullpath):
        fullpath_parts = fullpath.split('/')
        return all(any(fullpath_part.lower().startswith(subpath_part.lower()) for fullpath_part in fullpath_parts) for subpath_part in subpath.split('/'))
    # Sorry Windows
...
class Argument:
    ...
    def __init__(self, name=None, *, choices=None, completer=None, aliases=()):
        # Name is only necessary for the config type (i.e. as a parameter in @command)
        self.name = name
    ...
    def add_argument(self, parser, args, kwargs):
        if self.aliases:
            args += tuple(('-' if len(i) == 1 else '--') + i for i in self.aliases)
        if not 'help' in kwargs:
            kwargs['help'] = '<{}>'.format('|'.join(self.choices)) if self.choices else None
        argument = parser.add_argument(*args, **kwargs)
        if argcomplete is not None:
            if self.completer is not None:
                argument.completer = self.completer
            elif self.choices is not None:
                argument.completer = argcomplete.completers.ChoicesCompleter(self.choices)
...
class CommandObj:
    ...
    def execute(self, args=None):
        if CommandObj.exec_mode == 'execute':
            ...
            # subcommand propagation doesn't work for completing partial queries, but included
    ...
    def complete(self, args):
        ...
        try:
            with argcomplete.mute_stderr():
                parsed_args, remaining = finder._parser.parse_known_args(args, namespace=parsed_args)
        except BaseException as e:
            # argcomplete.warn('Exception in parsing args', e, traceback.format_exc())
            pass
        finder.completing = False

        # When the arguments are unmatched (e.g. --optional without a value), an exception will be
        # thrown and remaining will be None.
        ...
        if completer:
            func_args, func_kwargs = self.commandspec.get_args(vars(parsed_args), remaining)
            func_args = [arg if arg is not Parameter.empty else None for arg in func_args]
    ...
    def create_parser(self, partial=False):
        self.commandspec = CommandSpec(self.func, arguments=self.arguments, partial=partial)
    ...
    def execute_partial(self, args=None):
        ...
        parsed, remaining = self.parser.parse_known_args(args, namespace=parsed)
        func_args, func_kwargs = self.commandspec.get_args(vars(parsed), remaining)
        func_args = [arg if arg is not Parameter.empty else None for arg in func_args]
    ...
    def execute_with_list(self, args=None):
        '''
        Turns the given list of arguments (in argv format) into python parameters (*args, **kwargs)
        and run self.func
        '''

        # Defer CommandSpec initialization until command execution (lazy loading!)
        self.create_parser()
        ...
        if argcomplete is not None:
            _command = self
            completion_filter = getattr(self.unbound_func, 'completion_filter', prefix_filter)
            class CompletionFinder(argcomplete.CompletionFinder):
                def _get_completions(self, comp_words, cword_prefix, cword_prequote, first_colon_pos):
                    '''
                    Intercept argcomplete and allow attaching our own completers (which can be
                    attached to functions in addition to arguments)
                    '''
                    try:
                        global _command_complete
                        _command_complete = (cword_prefix, (lambda *_: None))

                        choices = _command.complete(args if args is not None else comp_words[1:])
                        completions = list(completion_filter(choices, cword_prefix))
            ...
            CompletionFinder()(self.parser, exclude=['--help', '-h'], always_complete_options=False)

        parsed, remaining = self.parser.parse(args if args is not None else sys.argv[1:])
        func_args, func_kwargs = self.commandspec.get_args(vars(parsed), remaining)
        return self.func(*func_args, **func_kwargs)
...
def MasterCommand(**kwargs):
    ...
    # Fill in the subcommand as the prog name so the help message shows the correct prog name
    for key, subcommand in kwargs.items():
        subcommand._command.prog = '{} {}'.format(_run._command.prog or Path(sys.argv[0]).name, key)
...
class Signature:
    ...
    def inspect(func):
        ...
        for arg, default in zipright(args, defaults, filler=Parameter.empty):
            parameters[arg] = Parameter(arg, Parameter.POSITIONAL_OR_KEYWORD,
                                        default=default, annotation=annotations.get(arg, Parameter.empty))
        if varargs:
            parameters[varargs] = Parameter(varargs, Parameter.VAR_POSITIONAL,
        ...
        for arg in kwonlyargs:
            ...
            parameters[arg] = Parameter(arg, Parameter.KEYWORD_ONLY,
                                        default=default, annotation=annotations.get(arg, Parameter.empty))
        if varkw:
            parameters[varkw] = Parameter(varkw, Parameter.VAR_KEYWORD,
...
def pad(iterator, n, filler=None):
    '''
    Returns an iterator of length n, built from the parameter iterator. The resulting iterator
    contains the first n elements if the iterator is longer than n, or padded with filler if
    the iterator is shorter.
    '''
    if iterator is None:
        return repeat(filler, n)
    return islice(chain(iterator, repeat(filler)), n)

def padleft(seq, n, filler=None):
    l = len(seq)
    return chain(repeat(filler, n-l), seq)

def zipright(seq1, seq2, filler=None):
    maxlen = max(len(seq1), len(seq2))
    return zip(padleft(seq1, maxlen, filler), padleft(seq2, maxlen, filler))
