    ...
    def getdoctest(self, file):
        ...
        for match in matches:
            ...
            if not expected.endswith('\n'):
                # docstring strips out the trailing newline. Add it back to match with the
                # shell output
                expected += '\n'
            # Remove prefix '> ', which is used to test outputs with blank lines
            expected = re.sub(r'^>( |$)', '', expected, flags=re.MULTILINE)
            command, expectations, *_ = command.split('#', 1) + [None]
            expectations = dict(e.split('=', 1) for e in expectations.split(' ') if e) if expectations else {}
        ...
        for t in TEST_FILES:
            with t.open('r') as f:
                for doctest in self.getdoctest(f):
                    with self.subTest(testfile=doctest.testfile, command=doctest.command):
                        print(doctest.describe())
                        p = self._Popen(doctest.command, cwd=str(t.parent))
                        stdout, stderr = p.communicate()
                        self.assertEqual(p.returncode, doctest.get_expectation('returncode', 0), msg=stderr)
                        doctest.make_assertion(self, stdout, stderr)

    def test_decorator(self):
        sys.path.append(os.getcwd())
        noarg = SourceFileLoader('module.name', str(TEST_DIR/'basic'/'no_arguments.py')).load_module()
...
class DocTest:
    ...
    def make_assertion(self, testcase, stdout, stderr):
        ...
        if self.get_expectation('glob', False):
            re_expected = re.compile('^' + re.escape(self.expected).replace('\\?', '.').replace('\\*', '.*') + '$')
    ...
    def describe(self):
        ...
        $ {command}
        {expected}
        '''.format(testfile=self.testfile, command=self.command, expected=self.expected)
